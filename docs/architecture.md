...
| `src/provider/credentials.rs` | `resolve_token(&ServiceCredentials)` — the single seam that turns a service's stored auth into a usable access token (REST + the agent's `GH_TOKEN`/`GITLAB_TOKEN`). `Pat` is wired (and carries GitLab's Group/Project Access Token bot identity, #10); **GitHub App (#9) minting lands here**. Unit-tested |
...
| `src/provider/{gitlab,github}/` | provider impls: REST calls for posting notes and `ensure_webhook` (idempotent hook registration — GET existing hooks, PUT/PATCH if our `/webhook/…` callback URL drifted, else create). Clients hold a `ServiceCredentials` and call `resolve_token` per request (so an app token can later be refreshed on expiry) |
...
- `git_services` — provider config: kind, base URL, bot username, token, webhook secret, `autofire` (when true, a newly-created task from this service's webhook is auto-confirmed — started running immediately instead of left pending for a manual confirm). the credential is a **type + value** pair: `auth_kind` (`pat`\|`app`, default `pat`) is the type, and `app_credentials` (JSONB, null for `pat`) is the value — the provider-specific secret bundle (GitHub App: `{app_id, private_key, installation_id}`). `app` is **groundwork for GitHub App (#9)** — stored and validated, but token minting is not implemented yet (see `provider::credentials` and [`docs/application-integration.md`](application-integration.md)); GitLab has no `app` flow. **GitLab uses `pat` with its own bot identity (#10)** — a **Group/Project Access Token** mints a `group_NNN_bot_*` service account, so the agent acts as itself, never the operator. **Required token scopes** (the token does git transport, note posting, and webhook registration): GitHub needs `repo` + `admin:repo_hook`; GitLab needs `api` + `write_repository` and Maintainer/Owner on the group/project. A GitLab Group/Project Access Token expires ≤365 days — rotate it (manually, or via the access-tokens API) before expiry.
