...
| `src/webhook/dispatch.rs` | event → `TriggerReason`, dedupe, project upsert, task create. Issue events fire per the service's **trigger mode** (`assignee`\|`label`\|`both`, see below). A comment on an issue/MR with an existing agent is **delivered as a message** to that task (one agent/session per issue/MR), not a new task — **regardless of trigger mode/label/assignee/mention**; start-gates only apply when no task exists yet. Issue/MR close stops the branch's live agent before reclaiming the worktree |
