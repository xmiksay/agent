...
| `GET`/`POST` | `/api/git_services` | tokens, webhook secrets, and `app_credentials` are write-only on GET; the view exposes `app_installed` (whether an `installation_id` is recorded) |
| `GET`/`PUT`/`DELETE` | `/api/git_services/{id}` | |
| `GET` | `/api/git_services/{id}/github_app/install` | returns `{ install_url }` — the GitHub App install URL (looked up via `GET /app`, with the service id as `state`). The SPA navigates the browser to it. Bearer-gated |
