...
| `src/workspace/git.rs` | `clone_or_fetch` + `HttpsAuth` (builds a token-HTTPS remote from the repo's SSH URL and a git credential helper that reads the token from env, so no secret lands in `.git/config`) |
