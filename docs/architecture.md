...
| `GET`/`PATCH`/`DELETE` | `/api/tasks/{id}` | detail + latest session; PATCH sets `task_state` to any of `pending\|working_on\|completed\|failed` on **any** task (operator override of the auto-advance), and edits `branch` only while `task_state == pending` — the branch may not equal the project's default branch (default branch is a project property now, not editable per task); DELETE force-kills if running |
