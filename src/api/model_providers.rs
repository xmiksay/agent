//! HTTP surface for the `/api/model_providers` catalog. The `api_key` is
//! write-only (like service tokens): never returned, only `has_api_key` is.
...
use crate::models::{ModelProvider, NewModelProvider, UpdateModelProvider};

/// A provider as the SPA sees it — secret-free. `has_api_key` reflects whether an
/// API-mode key is stored without revealing it; `kinds` lists the system-defined
/// backend keys a provider may use (so the form doesn't hardcode them).
#[derive(Serialize)]
...
pub async fn list(State(state): State<AppState>) -> Result<Json<ListResponse>, StatusCode> {
    let providers = state.model_provider_store.list().await.map_err(|e| {
...
pub async fn create(
    State(state): State<AppState>,
    Json(req): Json<NewModelProvider>,
...
pub async fn update(
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(req): Json<UpdateModelProvider>,
