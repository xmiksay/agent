...
- **SeaORM:** entity Model is the read shape; mutations go through `ActiveModel` + `Set(...)`. Most writes are single short statements; multi-statement delete-then-insert swaps (`set_trigger_models`, `set_trigger_configs`) run inside an explicit `db().begin()` … `commit()` transaction so a mid-swap failure rolls back (#65).
