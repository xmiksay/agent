...
// Workbench note: the live modal builds the trigger identically but submits via
// the tasks store. Here it just emits the built trigger so the form is testable.
const props = defineProps<{ open: boolean }>();
const emit = defineEmits<{ (e: "close"): void; (e: "created", id: string): void }>();

const list = projects;
const projectId = ref<string>(list[0]?.id ?? "");
const triggerKind = ref<TriggerKind>("issue");
const submitting = ref(false);

const fields = ref({ iid: 1, title: "", description: "", url: "", source_branch: "", target_branch: "", review_body: "", comment: "" });

const selectedProject = computed<ProjectListItem | undefined>(() => list.find((p) => p.id === projectId.value));
...
function buildTrigger(): TriggerReason {
  ...
  switch (triggerKind.value) {
    case "issue":
      return { type: "issue", iid: Number(f.iid), title: f.title, description: f.description, url: f.url };
    case "review_mr":
      return { type: "review_mr", iid: Number(f.iid), title: f.title, source_branch: f.source_branch, target_branch: f.target_branch || branchPlaceholder.value, url: f.url };
    case "fix_review":
      return { type: "fix_review", iid: Number(f.iid), title: f.title, source_branch: f.source_branch, url: f.url, review_body: f.review_body };
    case "mr_comment":
      return { type: "mr_comment", mr_iid: Number(f.iid), comment: f.comment, source_branch: f.source_branch, url: f.url };
    case "issue_comment":
      return { type: "issue_comment", issue_iid: Number(f.iid), comment: f.comment, url: f.url };
  }
}
...
<template>
  <div v-if="props.open" class="fixed inset-0 z-50 flex items-center justify-center bg-canvas/70 backdrop-blur-sm" @click.self="close">
      ...
      <div class="space-y-4 px-5 py-4">
        <div>
          ...
          <select v-model="projectId" class="select">
            <option v-for="p in list" :key="p.id" :value="p.id">{{ p.full_name }} ({{ p.provider }})</option>
          </select>
        </div>
        ...
        <div>
          ...
          <select v-model="triggerKind" class="select">
            <option v-for="k in triggerKinds" :key="k.value" :value="k.value">{{ k.label }} — {{ k.hint }}</option>
          </select>
        </div>

        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="label">{{ triggerKind === "issue_comment" ? "Issue iid" : triggerKind === "mr_comment" ? "MR iid" : "iid" }}</label>
        ...
        <div v-if="triggerKind === 'issue'">
          ...
          <textarea v-model="fields.description" rows="4" class="textarea font-mono" />
        </div>

        <div v-if="triggerKind === 'review_mr' || triggerKind === 'fix_review' || triggerKind === 'mr_comment'">
        ...
        <div v-if="triggerKind === 'mr_comment' || triggerKind === 'issue_comment'">
          ...
          <textarea v-model="fields.comment" rows="3" class="textarea font-mono" />
        </div>
      </div>
