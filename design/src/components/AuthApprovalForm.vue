...
//      buttons (single-select) or checkboxes (multi-select).
...
function toggleMulti(qi: number, label: string) {
  ...
  multiPicks.value[qi] = cur.includes(label) ? cur.filter((l) => l !== label) : [...cur, label];
}
...
const allAnswered = computed(() => {
  ...
  return qs.every((q, qi) =>
    q.multiSelect ? (multiPicks.value[qi] ?? []).length > 0 : !!singlePicks.value[qi],
  );
});
